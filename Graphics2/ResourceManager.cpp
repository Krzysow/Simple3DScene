...
shared_ptr<Mesh> ResourceManager::GetMesh(wstring modelName, XMMATRIX orientation, ColliderType colliderType)
{
	...
	else
	{
		...
		shared_ptr<Mesh> mesh = LoadModelFromFile(modelName, orientation, colliderType);
		if (mesh != nullptr)
		{
...
shared_ptr<Mesh> ResourceManager::LoadModelFromFile(wstring modelName, XMMATRIX orientation, ColliderType colliderType)
{
    ...
    for (unsigned int sm = 0; sm < scene->mNumMeshes; sm++)
    {
	    ...
	    {
			XMVECTOR prePosition = XMVectorSet(subMeshVertices->x, subMeshVertices->y, subMeshVertices->z, 1.0f);
			XMVECTOR postPosition = XMVector3Transform(prePosition, orientation);
			XMFLOAT3 position;
			XMStoreFloat3(&position, postPosition);
		...
		case none:
			...
			break;
		case sphere:
			boundingVolume = make_shared<SphereCollider>(modelVertices, numVertices);
			break;
		case box:
			//boundingVolume = make_shared<BoxCollider>(modelVertices, numVertices);
			break;
		}

	    shared_ptr<SubMesh> resourceSubMesh = make_shared<SubMesh>(vertexBuffer, indexBuffer, numVertices, numberOfIndices, material, boundingVolume);
