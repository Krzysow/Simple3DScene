...
void SkyNode::Render()
{
	...
	XMMATRIX worldTransformation = XMMatrixTranslationFromVector(DirectXFramework::GetDXFramework()->GetCamera()->GetCameraPosition());
	XMMATRIX completeTransformation = worldTransformation * DirectXFramework::GetDXFramework()->GetCamera()->GetViewMatrix() * DirectXFramework::GetDXFramework()->GetProjectionTransformation();
...
void SkyNode::BuildGeometryBuffers()
{
	...
	vertexBufferDescriptor.ByteWidth = sizeof(Vertex) * static_cast<UINT>(_numberOfVertices);
	...
	indexBufferDescriptor.ByteWidth = sizeof(UINT) * static_cast<UINT>(_numberOfIndices);
